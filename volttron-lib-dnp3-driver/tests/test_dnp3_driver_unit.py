...
from volttron.driver.interfaces.dnp3.dnp3 import Dnp3Register, Dnp3Driver
...
class TestStation:
    ...
    def test_station_init(self, master_app, outstation_app):
        ...
        reg_definition = {"Group": 0, "Variation": 0, "Index": 0}
        Dnp3Register(read_only=False, pointName="pointName", units="units", reg_type="reg_type", default_value=None,
                     description='',
                     master_application=master_app, reg_definition=reg_definition, )
...
def dnp3_inherit_init_args(csv_config, driver_config_in_json_config):
    ...
    #         'units': "",
    #         'read_only': ""}
    args = dict(read_only=False, pointName="pointName", units="units", reg_type="reg_type", default_value=None,
                description='',
                # master_application=master_app, reg_definition=reg_def
                )
    return args
...
def driver_config_in_json_config():
    ...
    """
    json_path = Path("./testing_data/dnp3-config.json")
...
class TestDNPRegister:
    ...
    def test_init(self, master_app, csv_config, dnp3_inherit_init_args):
        for reg_def in csv_config:
            # Dnp3Register(read_only=False, pointName="pointName", units="units", reg_type="reg_type", default_value=None,
            #              description='',
            #              master_application=master_app,
            #              reg_definition=reg_def,
            #              )
            Dnp3Register(**dnp3_inherit_init_args,
                         master_application=master_app,
                         reg_definition=reg_def,
                         )
    ...
    def test_get_register_value_analog_float(self, outstation_app, master_app, csv_config,
        ...
        for i in range(len(analog_input_val)):
            ...
            reg_def["Variation"] = "6"
            reg_def["Index"] = str(i)
            reg_defs.append(reg_def.copy())  # Note: Python gotcha, mutable don't evaluate til the end of the lo1op.
        print(reg_defs)
        ...
        for i, (val_update, reg_def) in enumerate(zip(analog_input_val, reg_defs)):
            ...
            dnp3_register = Dnp3Register(**dnp3_inherit_init_args,
                                         master_application=master_app,
                                         reg_definition=reg_def,
                                         )
