...
from dnp3_outstation.agent import Dnp3OutstationAgent as agent # agent
...
def main(parser=None, *args, **kwargs):
    ...
    # peer_method = "apply_update_analog_input"

    def get_db_helper():
        _peer_method = "display_outstation_db"
        _db_print = a.vip.rpc.call(peer, _peer_method).get(timeout=10)
        return _db_print

    def get_config_helper():
        _peer_method = "get_outstation_config"
        ...
        while True:
            if option == "ai":
                ...
                try:
                    ...
                    method = agent.Dnp3Agent.apply_update_analog_input
                ...
                try:
                    p_val = float(input_str.split(" ")[0])
                    index = int(input_str.split(" ")[1])
                    method = agent.Dnp3Agent.apply_update_analog_output
                    peer_method = method.__name__  # i.e., "apply_update_analog_input"
                    response = a.vip.rpc.call(peer, peer_method, p_val, index).get(timeout=10)
                ...
                try:
                    ...
                    index = int(input_str.split(" ")[1])
                    method = agent.Dnp3Agent.apply_update_binary_input
                ...
                try:
                    ...
                    if p_val_input not in ["0", "1"]:
                        ...
                        p_val = True if p_val_input == "1" else False
                    index = int(input_str.split(" ")[1])
                    method = agent.Dnp3Agent.apply_update_binary_output
                    peer_method = method.__name__
                    response = a.vip.rpc.call(peer, peer_method, p_val, index).get(timeout=10)
                ...
                # peer_method = "display_outstation_db"
                ...
                try:
                    ...
                    method = agent.Dnp3Agent.reset_outstation
                    peer_method = method.__name__
